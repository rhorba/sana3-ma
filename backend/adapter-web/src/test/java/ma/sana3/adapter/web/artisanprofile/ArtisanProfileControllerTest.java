...
class ArtisanProfileControllerTest {
    ...
    @MockitoBean
    private UpdateArtisanProfileHandler updateArtisanProfileHandler;

    @MockitoBean
    private GetArtisanProfileHandler getArtisanProfileHandler;

    private static ArtisanProfileResult stubResult(UUID userId) {
        Instant now = Instant.now();
        return new ArtisanProfileResult(UUID.randomUUID(), userId, "Fatima Zahra", "Pottery", "Fes", "Bio", "+212600000000", now, now);
    }

    private static org.springframework.test.web.servlet.request.RequestPostProcessor asArtisan(UUID userId) {
    ...
    private static org.springframework.test.web.servlet.request.RequestPostProcessor asBuyer(UUID userId) {
        return SecurityMockMvcRequestPostProcessors.authentication(new UsernamePasswordAuthenticationToken(
    ...
    void upsertWithValidRequestReturnsOk() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/artisan-profiles/me")
                        .with(csrf())
    ...
    void getReturnsProfileWhenPresent() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/artisan-profiles/me").with(asArtisan(userId)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.displayName").value("Fatima Zahra"));
    }
