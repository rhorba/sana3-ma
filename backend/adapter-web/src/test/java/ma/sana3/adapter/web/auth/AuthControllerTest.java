...
class AuthControllerTest {
    ...
    private static AuthResult stubResult() {
        return new AuthResult(UUID.randomUUID(), "user@example.com", Role.BUYER, "access-token", 900, "refresh-token");
    }
    ...
    void registerWithValidRequestReturnsCreatedWithRefreshCookie() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/register")
                        ...
                        .content(objectMapper.writeValueAsString(new RegisterRequest("user@example.com", "password123", RegistrableRole.BUYER))))
    ...
    void registerRejectsTooShortPassword() throws Exception {
        mockMvc.perform(post("/api/v1/auth/register")
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(new RegisterRequest("user@example.com", "short", RegistrableRole.BUYER))))
                .andExpect(status().isBadRequest())
    ...
    void registerWithDuplicateEmailReturnsConflict() throws Exception {
        when(registerUserHandler.handle(any())).thenThrow(new DuplicateEmailException("user@example.com"));
    ...
    void refreshWithInvalidTokenReturnsUnauthorized() throws Exception {
        when(refreshTokenHandler.handle(any())).thenThrow(new InvalidTokenException("Invalid or expired token"));
